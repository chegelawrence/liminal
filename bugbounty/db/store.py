...
class DataStore:
    ...
    async def get_live_hosts(self, scan_run_id: str) -> list[LiveHost]:
        ...
        return [
            LiveHost(
                ...
                title=r["title"],
                technologies=json.loads(r["technologies"]) if isinstance(r["technologies"], str) else list(r["technologies"]),
    ...
    def _row_to_finding(r: asyncpg.Record) -> Finding:
        return Finding(
            ...
            tags=json.loads(r["tags"]) if isinstance(r["tags"], str) else list(r["tags"]),
            cvss_score=r["cvss_score"],
            cve_id=r["cve_id"],
            raw_output=json.loads(r["raw_output"]) if isinstance(r["raw_output"], str) else dict(r["raw_output"]),
            is_false_positive=r["is_false_positive"],
            ai_analysis=r["ai_analysis"],
            poc_steps=r["poc_steps"],
            impact_statement=r["impact_statement"],
            remediation=r["remediation"],
            references=json.loads(r["references"]) if isinstance(r["references"], str) else list(r["references"]),
